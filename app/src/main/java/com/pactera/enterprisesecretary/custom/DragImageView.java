package com.pactera.enterprisesecretary.custom;
...
public class DragImageView extends AppCompatImageView {
	...
	private double beforeLenght, afterLenght;// 两触点距离

	private double scale_temp;// 缩放比例
	...
	void onTouchMove(MotionEvent event) {
		...
		/** 处理缩放 **/
			...
			double gapLenght = afterLenght - beforeLenght;// 变化的长度
	...
	class MyAsyncTask extends AsyncTask<Void, Integer, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			while (current_Width <= screen_W) {
				...
				onProgressUpdate(new Integer[] { left, top, right, bottom });//onProgressUpdate报错,可使用publishProgress
				try {
					Thread.sleep(10);
